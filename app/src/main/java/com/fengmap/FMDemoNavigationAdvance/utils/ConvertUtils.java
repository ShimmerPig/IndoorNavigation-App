...
public class ConvertUtils {
    ...
    public static float getAbsoluteDirectionWithVector(FMMapCoord vector) {
        ...
        double angle = calcTwoVectorIncludedAngleWithVector(vector, northVector);
     ...
     * @param vector2 坐标
     */
    public static double calcTwoVectorIncludedAngleWithVector(FMMapCoord vector1, FMMapCoord vector2) {
     ...
     * 获取地图旋转方向
     *
     * @param curVector  当前点坐标
     * @param lastVector 上个点坐标
     * @return
     */
    public static float getAbsoluteDirection(FMMapCoord curVector, FMMapCoord lastVector) {
        if (lastVector == null) {
            return 0;
        }
        FMMapCoord vector = new FMMapCoord(curVector.x - lastVector.x, curVector.y - lastVector.y);
        return getAbsoluteDirectionWithVector(vector);
    }
    ...
    public static String getDescription(ArrayList<FMNaviDescriptionData> datas, FMMapCoord mapCoord,
        ...
        int index = getPointIndex(datas, mapCoord, focusGroupId);
     ...
     * 获取坐标点在路径规划上的索引
     *
     * @param datas        路径描述路径所有集合
     * @param mapCoord     当前定位点
     * @param focusGroupId 焦点集合
     * @return
     */
    public static int getPointIndex(ArrayList<FMNaviDescriptionData> datas, FMMapCoord mapCoord,
                                    int focusGroupId) {
        int index = 0;
